...
- 下载好[qrsync 命令行同步工具](http://docs.qiniu.com/tools/v6/qrsync.html)，放在得出名为 qrsync 的 Unix executable格式文件，以下的教程只会用到 qrsync 文件。
- sublime
...
配置目的：能在 iTerm/Termial 中直接调用 qrsync
...
为了易于区分，我将 `conf.json` 重命名为  `cnfeat.json`
